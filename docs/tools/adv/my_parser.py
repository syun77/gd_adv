...
from node.float    import Float
from node.string   import String
...
class MyParser:
    ...
    def __init__(self, lexer, funcfile, defines):
        ...
        for fDefine in defines.split(","):
            ...
            f = open(fDefine, "r", encoding='utf_8')
            data = yaml.load(f, Loader=yaml.SafeLoader)
            if "consts" in data: # 定数
                self.defines.update(data["consts"])
            if "vars" in data: # 変数の定数
                self.lexer.varDefines.update(data["vars"])
            if "bits" in data: # フラグの定数
                self.lexer.bitDefines.update(data["bits"])
            if "items" in data: # アイテム定数
                self.defines.update(data["items"])
            if "scenes" in data: # シーン定数
                self.lexer.sceneDefines.update(data["scenes"])
            if "sysbits" in data: # システムフラグ定数
                self.lexer.sysbitDefines.update(data["sysbits"])
            f.close()
 ...
 #       return ttype == '*' or ttype == '/' or ttype == TokenType.AND
    ...
    def parseFactor(self):
        ...
        if ttype == TokenType.INT:
            ...
            node = Float(self.lexer.token)
            self.lookAhead() # skip FLOAT
        elif ttype == TokenType.STRING:
            node = String(self.lexer.token)
            self.lookAhead() # skip STRING
